...
class ProgressBar
{
	...
	public function reset($formatstring, $bar, $prefill, $width, $target_num, $options = array())
	{
		...
		$default_options = array('percent_precision' => 2, 'fraction_precision' => 0, 'percent_pad' => ' ', 'fraction_pad' => ' ', 'width_absolute' => true, 'ansi_terminal' => false, 'ansi_clear' => false, 'num_datapoints' => 5, 'min_draw_interval' => 0.0,);
		...
		$transitions = array('%%' => '%%', '%fraction%' => $cur . '/' . $max, '%current%' => $cur, '%max%' => $max, '%percent%' => $perc . '%%', '%bar%' => '%1$s', '%elapsed%' => '%5$s', '%estimate%' => '%6$s',);
	...
	public function display($current)
	{
		...
		$this->_rlen = printf($this->_skeleton, $visbar, $current, $this->_target_num, $percent * 100, $elapsed, $estimate);
	...
	public function erase($clear = false)
	{
		if ($this->_options['ansi_terminal'] and ! $clear) {
			...
			echo str_repeat(chr(0x08), $this->_rlen), str_repeat(chr(0x20), $this->_rlen), str_repeat(chr(0x08), $this->_rlen);
		}
	}
}
