...
class SecurityController extends ApiController
{
     ...
     * @ApiDoc(
     *   section = "OAuth",
     *   description = "Set the expo token that will be used to send push notifications to the device",
     *   parameters = {
     *      {
     *          "name" = "name",
     *          "required" = true,
     ...
     *          "required" = true,
     *          "dataType" = "string",
     *          "description" = "The device uid to associate the token"
     *      }
     *   }
     * )
     *
     * @Route("/push-token", name="oauth_push_token")
     * @Method({"POST"})
     * @Template()
     */
    public function setPushToken(Request $request)
    {
        ...
        if (!$request->query->has('name') || mb_strlen(trim($request->query->get('name'))) < 3 || 
            !$request->query->has('token') || mb_strlen(trim($request->query->get('token'))) < 3 || 
            !$request->query->has('device_uid') || mb_strlen(trim($request->query->get('device_uid'))) < 3
        ) {
            return $this->format(['error' => 'Le token n\'a pas pu être ajouté car les parametres donné par l\'application ne sont pas valide. Contactez l\'auteur de l\'application.'], 400, [], $request);
        }

        // Check if user is logged in and can use external applications
        if (!$this->isGranted('IS_AUTHENTICATED_FULLY') && !$this->isGranted('IS_AUTHENTICATED_REMEMBERED')) {
            return $this->format(['error' => $this->get('translator')->trans('user.api_login.login', ['%name%' => $request->query->get('name')])], 400, [], $request);
        } elseif (!$this->isGranted('ROLE_API_USE')) {
            return $this->format(['error' => $this->get('translator')->trans('user.api_login.orga')], 400, [], $request);
        }
        $this->denyAccessUnlessGranted('ROLE_API_USE');

        //update pushToken
        $em->createQuery('UPDATE EtuCoreApiBundle:OauthClient c
                          SET c.pushToken = :token
                          WHERE (c.name = :name
                          and c.deviceUID = :deviceUID)')
            ->setParameter('name', $request->query->get('name'))
            ->setParameter('deviceUID', $request->query->get('device_uid'))
